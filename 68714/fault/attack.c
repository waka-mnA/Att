...
int keySto3[MAX_NUM]={0}; //storage for byte 11, 2,  16, 4
int keySto4[MAX_NUM]={0}; //storage for byte 14, 5,  9,  7
//Storage for second fault ciphertext
int keySto1_2[MAX_NUM]={0};
...
int compareKeys(uint8_t* key){
  int count=0;
  for (int i = 0;i<MAX_NUM;i++){
    if (keySto1[i]== -1 || keySto2[i]== -1 || keySto3[i]== -1 || keySto4[i]== -1 ) break;
    for (int j = 0;j<MAX_NUM;j++){
      if (keySto1_2[j]== -1 || keySto2_2[j]== -1 || keySto3_2[j]== -1 || keySto4_2[j]== -1 ) break;
      if (keySto1[i] != keySto1_2[j])continue;
      if (keySto2[i] != keySto2_2[j])continue;
      if (keySto3[i] != keySto3_2[j])continue;
      if (keySto4[i] != keySto4_2[j])continue;
      count++;
      if (count == 1) {
        key[0] = keySto1[i];
        key[1] = keySto2[i];
...
void findK(uint8_t c1, uint8_t c2, uint8_t c3, uint8_t c4,
    uint8_t cf1, uint8_t cf2, uint8_t cf3, uint8_t cf4,
...
//Inverse Key Schedule function to recover original key from 10th round key
 void invKeySchedule(uint8_t key[OCTET], uint8_t rcon)
{
    uint8_t * key_0 = key + OCTET - 4;
    uint8_t * key_m1 = key_0 - 4;
